...
export const addChemical = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      const response = await fetch("/api/v1/chemical", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
...
export const addBatchNumber = createAsyncThunk(
  ...
  async ({ chemical, batchNo }, { rejectWithValue }) => {
    try {
      const response = await fetch(`/api/v1/chemical/${chemical}/batch`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
export const deleteChemical = createAsyncThunk(
  ...
  async (chemical, { rejectWithValue }) => {
    try {
      const response = await fetch(`/api/v1/chemical/${chemical}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      });
      if (!response.ok) {
...
export const quoteSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getChemicals.fulfilled, (state, { payload }) => {
        state.loading = false;
        state.chemicals = payload.data;
        toast.success("Chemicals fetched successfully");
      })
      ...
      .addCase(addBatchNumber.fulfilled, (state, { payload }) => {
        state.loading = false;
        const index = state.chemicals.findIndex(
          (c) => c.chemical === payload.data.chemical
        );
        if (index !== -1) state.chemicals[index] = payload.data;
        toast.success("Batch number added successfully");
      })
      .addCase(addBatchNumber.rejected, (state, { payload }) => {
        state.loading = false;
        toast.error(payload.message);
      })
      .addCase(addChemical.fulfilled, (state, { payload }) => {
        state.loading = false;
        state.chemicals.push(payload.data);
